...
qualDat <- read_xlsx('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\Snow Performance\\2025_Mech_LowZonePFW_Ride\\2025_Mech_LowZonePFW_Ride_Qualitative.xlsx')

# # Load in the IMU data
imuDat <- read.csv('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\Snow Performance\\2025_Mech_LowZonePFW_Ride\\IMU\\2_IMUOutcomes.csv') 



# Pressure from Insoles - during carving
pressDat <- read.csv('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\Snow Performance\\2025_Mech_LowZonePFW_Ride\\Xsensor\\Exported\\0_CompiledResults_2.csv')
...
gpsDat <- read.csv('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\Snow Performance\\2025_Mech_LowZonePFW_Ride\\RaceBox GPS\\2_GPSOutcomes.csv')



baseline <- 'CFS' # baseline configuration

otherConfigs <- 'PFS' # other configurations tested against base
...
qualDat %>%
  pivot_longer(cols = Stiffness:FreeRiding, 
...
withinSubPlotAvg(qualDat, 'OverallFit','higher','Overall Rating') 
...
heelAvg_speedAngle <- imuDat%>% 
  group_by(Subject, Config, Order)%>%
  summarize(
    BA_HeelTurn_Mean = mean(BoardAngle_HeelTurns, na.rm = TRUE),
  )

toeAvg_speedAngle <- imuDat%>% 
  group_by(Subject, Config, Order)%>%
  summarize(
    BA_ToeTurn_Mean = mean(BoardAngle_ToeTurns, na.rm = TRUE),
  )
...
heelspdat <- merge(x=heelAvg_speedAngle ,y=avgDat,all=TRUE) 


toespdat <- merge(x=toeAvg_speedAngle ,y=avgDat,all=TRUE)
...
#### Models factoring in Heel  & Toe Turn Angles


# Note for trail run metrics: Warmup was reduced (from 1000 to 400)
heelspdatMod <- brm(data = heelspdat, # Bayes model
              family = gaussian,
              z_score ~ Config + BA_HeelTurn_Mean + (1+ Config| Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
                        prior(cauchy(0, 1), class = sd), #This is a regularizing prior, meaning we will allow the SD of the betas to vary across subjects
                        prior(cauchy(0, 1), class = sigma)), #overall variability that is left unexplained 
...
# p <- withinSubPlotAvg(heelspdat, colName = 'AvgSpeed', dir = 'higher','Average Speed (kph)')


extractVals(heelspdat, heelspdatMod , otherConfigs, baseline, 'AvgSpeed', 'higher') 
...
# Note for trail run metrics: Warmup was reduced (from 1000 to 400)
toespdatMod <- brm(data = toespdat, # Bayes model
              family = gaussian,
              z_score ~ Config +  BA_ToeTurn_Mean + (1+ Config| Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
                        prior(cauchy(0, 1), class = sd), #This is a regularizing prior, meaning we will allow the SD of the betas to vary across subjects
                        prior(cauchy(0, 1), class = sigma)), #overall variability that is left unexplained 
...
extractVals(toespdat, toespdatMod , otherConfigs, baseline, 'AvgSpeed', 'higher') 
```
...
heel_speedAngle <- imuDat%>% 
  group_by(Subject, Config, Order)%>%
  summarize(
    BA_HeelTurn_Mean = mean(BoardAngle_HeelTurns, na.rm = TRUE),
  )

toe_speedAngle <- imuDat%>% 
  group_by(Subject, Config, Order)%>%
  summarize(
    BA_ToeTurn_Mean = mean(BoardAngle_ToeTurns, na.rm = TRUE),
  )
...
topHeelSpDat <- merge(x=heel_speedAngle ,y=topDat,all=TRUE) 


topToeSpDat <- merge(x=toe_speedAngle ,y=topDat,all=TRUE)
...
# Note for trail run metrics: Warmup was reduced (from 1000 to 400)
topHeelSpDatMod <- brm(data = topHeelSpDat, # Bayes model
              family = gaussian,
              z_score ~ Config + BA_HeelTurn_Mean + (1+ Config| Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
...
extractVals(topHeelSpDat, topHeelSpDatMod  , otherConfigs, baseline, 'TopSpeed', 'higher') 
...
topToeSpDatMod <- brm(data = topToeSpDat , # Bayes model
              family = gaussian,
              z_score ~ Config +  BA_ToeTurn_Mean + (1+ Config| Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
...
extractVals(topToeSpDat, topToeSpDatMod, otherConfigs, baseline, 'TopSpeed', 'higher') 
...
# Look at histogram of the data if any outliers need to be removed
ggplot(data = heelEdat, aes(x = BoardAngle_HeelTurns, color = Config)) + geom_histogram() + facet_wrap(~Subject)



heelEdat<- subset(heelEdat, heelEdat$z_score < 2) #removing outliers
heelEdat<- subset(heelEdat, heelEdat$z_score > -2)
...
# Look at histogram of the data if any outliers need to be removed
ggplot(data = toeEdat, aes(x = BoardAngle_ToeTurns, color = Config)) + geom_histogram() + facet_wrap(~Subject)


toeEdat<- subset(toeEdat, toeEdat$z_score < 2) #removing outliers
toeEdat<- subset(toeEdat, toeEdat$z_score > -2)
...
heelRFDdat<- subset(heelRFDdat, heelRFDdat$z_score < 2) #removing outliers
heelRFDdatP<- subset(heelRFDdat, heelRFDdat$z_score > -2)
...
toeTurnDat <- imuDat%>% 
  group_by(Subject, Config, Order)%>%
  summarize(
    BA_TorTurn_Mean = mean(BoardAngle_ToeTurns, na.rm = TRUE),
  )
...
heeldat <- merge(x=heeldat1 ,y=toeTurnDat,all=TRUE)
...
heeldat<- subset(heeldat, heeldat$z_score < 2) #removing outliers
heeldat<- subset(heeldat, heeldat$z_score > -2)

heelmod <- brm(data = heeldat, 
              family = gaussian,
              z_score ~ Config + BA_TorTurn_Mean + (1 | Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
                        prior(cauchy(0, 1), class = sd), #This is a regularizing prior, meaning we will allow the SD of the betas to vary across subjects
                        prior(cauchy(0, 1), class = sigma)), #overall variability that is left unexplained 
...
Toe Contact Area During Heel Turns
...
# Look at histogram of the data if any outliers need to be removed
ggplot(data = Toedat, aes(x = ToeAreaP, color = Config)) + geom_histogram() + facet_wrap(~Subject)


heeldat<- subset(heeldat, heeldat$z_score < 2) #removing outliers
heeldat<- subset(heeldat, heeldat$z_score > -2)

ToedatMod <- brm(data = Toedat, 
              family = gaussian,
...
p <- withinSubPlotAvg(Toedat, colName = 'ToeAreaP', dir = 'higher','Toe Contact Area - Heel Turns (%)')


extractVals(Toedat, ToedatMod, otherConfigs, baseline, 'ToeAreaP', 'higher')
```
