...
ev_COMpos <- extractVals(COMp, COMwMod, otherConfigs, baseline, 'COMWork_pos', 'lower')
es_COMpos <- EffectSizeLM(COMp, 'COMWork_pos ~ Config + Order + (1 + Config|Subject)')
checkEffectMagnitude(ev_COMpos, es_COMpos, baseline)
```
...
ev_COMneg <- extractVals(COMn, NCOMwMod, otherConfigs, baseline, 'COMWork_neg', 'higher')
es_COMneg <- EffectSizeLM(COMn, 'COMWork_neg ~ Config + Order + (1 + Config|Subject)')
checkEffectMagnitude(ev_COMneg, es_COMneg, baseline)
```
...
ev_Heel <- extractVals(HeelCondat, HeelMod, otherConfigs, baseline, 'heelAreaP', 'higher')
es_Heel <- EffectSizeLM(HeelCondat, 'heelAreaP ~ Config + (Config|Subject)')
...
es_Toe <- EffectSizeLM(Toedat, 'maxmaxToes ~ Config + (Config|Subject)')
