...
ev_COM_UH <- extractVals(psCOM_UH, psCOM_UH_mod, otherConfigs, baseline, 'COMWork_pos', 'lower')

es_COM_UH <- EffectSizeLM(psCOM_UH,'COMWork_pos ~ Config + (Config|Subject)')
checkEffectMagnitude(ev_COM_UH, es_COM_UH, baseline)
```
...
ev_COM_DH <- extractVals(ngCOM_DH, ngCOM_DH_mod, otherConfigs, baseline, 'COMWork_neg', 'higher')

es_COM_DH <- EffectSizeLM(ngCOM_DH, 'COMWork_neg ~ Config + (1 + Config | Subject)')
checkEffectMagnitude(ev_COM_DH, es_COM_DH, baseline)
```
...
es_Speed <- EffectSizeLM(Speeddat, 'imuSpeed ~ Config + (1 + Config | Subject)')
...
ev_Toe <- extractVals(PeakToeWalkdat, PeakToeWalkdatMod, otherConfigs, baseline, 'maxmaxToes', 'lower')

es_Toe <- EffectSizeLM(PeakToeWalkdat, 'maxmaxToes ~ Config + (1 + Config | Subject)')
...
ev_Ever <- extractVals(Everdat, EverdatMod, otherConfigs, baseline, 'pIEgyro', 'lower')

es_Ever <- EffectSizeLM(Everdat, 'pIEgyro ~ Config + imuSpeed + (1 + Config | Subject)')
checkEffectMagnitude(ev_Ever, es_Ever, baseline)
...
ev_KABD <- extractVals(KABDOGLandDat, KABDOGLandDatMod, otherConfigs, baseline, 'RkneeABDMom', 'higher')

es_KABD <- EffectSizeLM(KABDOGLandDat, 'RkneeABDMom ~ Config + (1 + Config | Subject)')
checkEffectMagnitude(ev_KABD, es_KABD, baseline)
```
...
ev_TimeUnLand <- extractVals(TimeUnLandDat, TimeUnLandDatMod, otherConfigs, baseline, 'StabalizeTime', 'lower')

es_TimeUnLand <- EffectSizeLM(TimeUnLandDat, 'StabalizeTime ~ Config + (1 + Config | Subject)')
checkEffectMagnitude(ev_TimeUnLand, es_TimeUnLand, baseline)
```
...
ev_Heel <- extractVals(HeelCondat, HeelCondatMod, otherConfigs, baseline, 'heelAreaP', 'higher')

es_Heel <- EffectSizeLM(HeelCondat, 'heelAreaP ~ Config + (1 + Config | Subject)')
checkEffectMagnitude(ev_Heel, es_Heel, baseline)
```
...
ev_DorsalMax <- extractVals(peakDorsal, DorsalMaxMod, otherConfigs, baseline, 'maxDorsalPressure', 'lower')

es_DorsalMax <- EffectSizeLM(peakDorsal, 'maxDorsalPressure ~ Config + (1 + Config | Subject)')
checkEffectMagnitude(ev_DorsalMax, es_DorsalMax, baseline)
...
ev_DorsalMean <- extractVals(avgDorsal, DorsalMeanMod, otherConfigs, baseline, 'meanDorsalPressure', 'higher')

es_DorsalMean <- EffectSizeLM(avgDorsal, 'meanDorsalPressure ~ Config + (1 | Subject)')
checkEffectMagnitude(ev_DorsalMean, es_DorsalMean, baseline)
