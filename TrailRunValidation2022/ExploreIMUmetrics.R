...
dat <- read.csv(file = 'C:/Users/eric.honert/Boa Technology Inc/PFL Team - General/Testing Segments/EndurancePerformance/TrailRun_2022/OutdoorData/IMUmetrics.csv')
...
sumdat1 <- dat1 %>%
  ...
  summarise(pIEgyro = mean(imuSpeed)) %>%
  pivot_wider(names_from = Config, values_from = pIEgyro)
sumdat1$perc <- (sumdat1$lace-sumdat1$pfs)/sumdat1$lace*100
