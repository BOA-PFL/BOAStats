...
dat <- dat %>% 
  ...
  group_by(Subject) %>%
  mutate(z_score = scale(timeToStab)) %>% 
  group_by(Config)

#dat<- subset(dat, dat$z_score < 2) #removing outliers  
#dat<- subset(dat, dat$z_score > -2)

ggplot(data = dat, aes(x = timeToStab, fill = Config)) + geom_histogram() + facet_wrap(~Subject) 

p<- withinSubPlot(dat, colName = 'timeToStab', dir = 'lower')

p + ylab('Time to stabilization (ms)')
...
dat <- dat %>% 
  filter(steadiness < 100)%>%
  group_by(Subject) %>%
  mutate(z_score = scale(steadiness)) %>% 
  group_by(Config)

#dat<- subset(dat, dat$z_score < 2) #removing outliers  
#dat<- subset(dat, dat$z_score > -2)

ggplot(data = dat, aes(x = steadiness, fill = Config)) + geom_histogram() + facet_wrap(~Subject) 

p<- withinSubPlot(dat, colName = 'steadiness', dir = 'lower')
...
extractVals(dat, runmod, otherConfigs, 'steadiness', 'lower') 
