...
baseConfig <- 'DD' # baseline config

otherConfigs <- c('LZ', 'UZ') # list configs being tested against baseline
...
ggplot(data = dat, aes(x = timeToStab, color = Config)) + geom_histogram() + facet_wrap(~Subject) 



p <- withinSubPlot(dat, colName = 'timeToStab', dir = 'lower')
p+ ylab('time to stabilize (frames)')
