...
outcomes = pd.DataFrame({'Power_steady':np.mean(steadyPower), 'Cadence_steady': np.mean(steadyCadence), 
                           'Symmetry_steady': np.mean(steadySym), 'Power_sprint':sprintMax, 
                           'Cadence_sprint':cadenceMax, 'Symmetry_sprint':symmetryMax}, index = [0])  



#Adding compiled data to Excel sheet - Change the file path to correct place and csv
outcomes.to_csv("C:/Users/bethany.kilpatrick/Boa Technology Inc/PFL - General/Cycling2021/CompiledPowerData.csv",mode='a',header=True)
