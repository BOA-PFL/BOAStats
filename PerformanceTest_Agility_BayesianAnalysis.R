...
dat$Config <- factor(dat$Config, c('LD', 'ED', 'PD'))
...
cmjDat <- cmjDat %>% 
  filter(CT > 0.1) %>% #remove values with impossible contact time
  filter(CT < 1.5) %>%
  group_by(Subject) %>%
...
p <- withinSubPlot(cmjDat, colName = 'impulse', dir = 'higher')
...
p<-withinSubPlot(skaterDat, colName = 'impulse', dir = 'higher')
p + ylab('Impulse (Ns)')
