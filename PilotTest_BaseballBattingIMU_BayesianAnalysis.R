...
dat <- rename(dat, 'BatSpeed_mph'='Bat Speed (mph)')
dat$BatSpeed_mph <- as.numeric(dat$BatSpeed_mph)

#grouping by subject and config by the variable being observed
dat <- dat %>% 
  group_by(Subject) %>%
  mutate(z_score = scale(BatSpeed_mph)) %>% 
  group_by(Config)


#Normalization histograms, Check for normalish distribution/outliers
ggplot(data = dat, aes(x = BatSpeed_mph, color = Config)) + geom_histogram() + facet_wrap(~Subject) #Change x axis variable to your variable of interest. Check for normal-ish distribution.
...
withinSubPlot(dat, colName = 'BatSpeed_mph', dir = 'higher') 
...
################### Bat Rotational Acceleration

dat <- rename(dat, 'BatRotAcc_g'='Rotational Acceleration (g)')
dat$BatRotAcc_g <- as.numeric(dat$BatRotAcc_g)

#grouping by subject and config by the variable being observed
dat <- dat %>% 
  group_by(Subject) %>%
  mutate(z_score = scale(BatRotAcc_g)) %>% 
  group_by(Config)


#Normalization histograms, Check for normalish distribution/outliers
ggplot(data = dat, aes(x = BatRotAcc_g, color = Config)) + geom_histogram() + facet_wrap(~Subject) #Change x axis variable to your variable of interest. Check for normal-ish distribution.

# "best of" Line graph 
# This graph shoes a "Snap shot" of subject's best trial in each shoe. This is for demonstration purposes only, try to not take this graph too literally 
withinSubPlot(dat, colName = 'BatRotAcc_g', dir = 'higher') 
...
dat <- rename(dat, 'PlaneScore'='Plane Score')
dat$PlaneScore <- as.numeric(dat$PlaneScore)

#grouping by subject and config by the variable being observed
dat <- dat %>% 
  group_by(Subject) %>%
  mutate(z_score = scale(PlaneScore)) %>% 
  group_by(Config)


#Normalization histograms, Check for normalish distribution/outliers
ggplot(data = dat, aes(x = PlaneScore, color = Config)) + geom_histogram() + facet_wrap(~Subject) #Change x axis variable to your variable of interest. Check for normal-ish distribution.

# "best of" Line graph 
# This graph shoes a "Snap shot" of subject's best trial in each shoe. This is for demonstration purposes only, try to not take this graph too literally 
withinSubPlot(dat, colName = 'PlaneScore', dir = 'higher') 
...
################### Attack Angle ~10 deg is optimal

dat <- rename(dat, 'AttackAngle'='Attack Angle (deg)')
dat$AttackAngle <- as.numeric(dat$AttackAngle)

#grouping by subject and config by the variable being observed
dat <- dat %>% 
  group_by(Subject) %>%
  mutate(z_score = scale(AttackAngle)) %>% 
  group_by(Config)


#Normalization histograms, Check for normalish distribution/outliers
ggplot(data = dat, aes(x = AttackAngle, color = Config)) + geom_histogram() + facet_wrap(~Subject) #Change x axis variable to your variable of interest. Check for normal-ish distribution.

# "best of" Line graph 
# This graph shoes a "Snap shot" of subject's best trial in each shoe. This is for demonstration purposes only, try to not take this graph too literally 
withinSubPlot(dat, colName = 'AttackAngle', dir = 'higher') 
...
# # Output of the Confidence Interval
extractVals(dat, runmod, otherConfigs, 'AttackAngle', 'higher') 
