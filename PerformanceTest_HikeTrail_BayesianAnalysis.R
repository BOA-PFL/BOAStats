...
dat <- subset(dat, dat$Sesh == 'trail')
###### peak jerk

dat <- dat %>% 
  
  group_by(Subject) %>%
  mutate(z_score = scale(pJerk)) %>% 
  group_by(Config)

#dat<- subset(dat, dat$z_score < 2) #removing outliers  
#dat<- subset(dat, dat$z_score > -2)

ggplot(data = dat, aes(x = pJerk, fill = Config)) + geom_histogram() + facet_wrap(~Subject) 

p<- withinSubPlot(dat, colName = 'pJerk', dir = 'lower')

p + ylab('Peak Jerk (m/s^3)')
...
extractVals(dat, runmod, otherConfigs, 'pJerk', 'lower') 
...
dat <- dat %>% 
  ...
  group_by(Subject) %>%
  mutate(z_score = scale(rMLacc)) %>% 
  group_by(Config)

#dat<- subset(dat, dat$z_score < 2) #removing outliers  
#dat<- subset(dat, dat$z_score > -2)

ggplot(data = dat, aes(x = rMLacc, fill = Config)) + geom_histogram() + facet_wrap(~Subject) 

p<- withinSubPlot(dat, colName = 'rMLacc', dir = 'lower')

p + ylab('ML acceleration range (m/s^2)')
...
extractVals(dat, runmod, otherConfigs, 'rMLacc', 'lower') 
...
ggplot(data = dat, aes(x = pIEgyro, fill = Config)) + geom_histogram() + facet_wrap(~Subject) 

p<- withinSubPlot(dat, colName = 'pIEgyro', dir = 'lower')

p + ylab('Peak eversion velocity (deg/s)')
...
extractVals(dat, runmod, otherConfigs, 'pIEgyro', 'lower')
...
ggplot(data = dat, aes(x = imuSpeed, fill = Config)) + geom_histogram() + facet_wrap(~Subject) 

p<- withinSubPlot(dat, colName = 'imuSpeed', dir = 'higher')

p + ylab('Foot speed (m/s)')
...
extractVals(dat, runmod, otherConfigs, 'imuSpeed', 'higher')
