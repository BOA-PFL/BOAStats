...
ev_EE <- extractVals(EEDat, EEDatMod, otherConfigs, baseline, 'EE', 'lower')

es_EE <- EffectSizeLM(EEDat,'EE ~ Config + (Config|Subject)')
checkEffectMagnitude(ev_EE, es_EE, baseline)
```
...
ev_Sprint <- extractVals(sprintDat, sprintDatMod, otherConfigs, baseline, 'Power_sprint', 'higher')

es_Sprint <- EffectSizeLM(sprintDat, 'Power_sprint ~ Config + Order + (Config|Subject)')
checkEffectMagnitude(ev_Sprint, es_Sprint, baseline)
```
...
ev_MfPress <- extractVals(mfPDat, mfPMod, otherConfigs, baseline, 'mfDorsalPressure', 'higher')

es_MfPress <- EffectSizeLM(mfPDat, 'mfDorsalPressure ~ Config + (1|Subject)')
checkEffectMagnitude(ev_MfPress, es_MfPress, baseline)
...
ev_FfPress <- extractVals(ffPDat, ffPMod, otherConfigs, baseline, 'ffDorsalPressure', 'higher')

es_FfPress <- EffectSizeLM(ffPDat, 'ffDorsalPressure ~ Config + (1|Subject)')
checkEffectMagnitude(ev_FfPress, es_FfPress, baseline)
```
