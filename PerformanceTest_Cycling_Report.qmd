...
pressDat <- subset(pressDat, pressDat$Movement == 'Sprint')
...
dat <- dat %>% 
  mutate(Config = replace(Config, Config == 'DD-HL', 'DDHL'))%>%
  mutate(Config = replace(Config, Config =='PFS-SD', 'PFSSD'))
...
baseline <- 'PFSSD' # baseline configuration

otherConfigs <- c('DDHL', 'DD') # other configurations tested against base
...
# extractVals <- function(dat = dat, mod = datMod, configNames = , baseConfig, var, dir) {  
...
# Forefoot ave pressure in Late "stance"
ffPDat<- pressDat %>% 
  group_by(Config) %>%
  mutate(z_score = scale(ffP_late)) %>% 
  group_by(Config)
...
p <- withinSubPlot(ffPDat, colName = 'ffP_late', dir = 'higher')
p + ylab('Ave Forefoot Pressure (kPa)')

extractVals(ffPDat, ffPMod, otherConfigs, baseline, 'ffP_late', 'higher') 
```
...
################
###############################################
# Midfoot ave pressure in Late "stance"
mfPDat<- pressDat %>% 
  group_by(Config) %>%
  mutate(z_score = scale(mfP_late)) %>% 
  group_by(Config)
...
extractVals(mfPDat, mfPMod, otherConfigs, baseline, 'mfP_late', 'higher') 
