...
dat <- read.csv('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\AgilityPerformanceData\\AS_Court_WomensPanelStiffnessI_Mech_Jan25\\Overground\\0_CompiledAgilityDataTest.csv') 



dat <- dat %>%
  mutate(Order = replace(Order, Order == '1- PerformanceTestData_V2','1'))%>%
  mutate(Order = replace(Order, Order == '2 - PerformanceTestData_V2','2'))%>% 
  mutate(Order = replace(Order, Order == '3- PerformanceTestData_V2','3'))%>%
  mutate(Order = replace(Order, Order == '4- PerformanceTestData_V2','4'))%>%
  mutate(Order = replace(Order, Order == '5- PerformanceTestData_V2','5'))%>%
  mutate(Order = replace(Order, Order == '6- PerformanceTestData_V2','6'))


# load in pressure data
Press<- read.csv('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\AgilityPerformanceData\\AS_Court_WomensPanelStiffnessI_Mech_Jan25\\Xsensor\\Cropped\\0_CompiledResults_4.csv')

# load in treadmill data
treadmill <- read.csv('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\AgilityPerformanceData\\AS_Court_WomensPanelStiffnessI_Mech_Jan25\\Treadmill\\TreadmillOutcomes.csv')

static <- read.csv('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\AgilityPerformanceData\\AS_Court_WomensPanelStiffnessI_Mech_Jan25\\Xsensor\\Cropped\\Static\\0_CompiledResults_Static.csv')

#Load in Compiled Qualitative Sheet
# qualDat <- read_xlsx(file.choose())
qualDat<- read_xlsx('C:\\Users\\bethany.kilpatrick\\BOA Technology Inc\\PFL Team - General\\Testing Segments\\AgilityPerformanceData\\AS_Court_WomensPanelStiffnessI_Mech_Jan25\\AS_Court_WomensPanelStiffnessI_Mech_Jan25.xlsx')





baseline <- 'LS' # baseline configuration

otherConfigs <- c('MS', 'HS') # other configurations tested against base
...
qualDat %>%
  ...
  summarize(
    avg = round(mean(Rating, na.rm = TRUE),1)
...
qualDat %>%
  group_by(Config)%>%
  summarize(
    R_Torque_Prox = round(mean(R_DialTorque1, na.rm = TRUE),2),
    # R_Torque_Dist = mean(R_DialTorque2, na.rm = TRUE),
    L_Torque_Prox = round(mean(L_DialTorque1, na.rm = TRUE),2)
...
cmjDatPP<- subset(cmjDatPP, cmjDatPP$z_score < 2) #removing outliers
cmjDatPP<- subset(cmjDatPP, cmjDatPP$z_score > -2)
...
withinSubPlotAvg(cmjDatPP, 'peakPFmom', 'higher', 'Peak Plantarflexion Moment (Nm)')
...
ggplot(data = skaterDatPow, aes(x = peakPower, color = Config)) + geom_histogram() + facet_wrap(~Subject) 
...
COMwMod <- brm(data = COMp, # Bayes model
              family = gaussian,
              z_score ~ Config + Sesh + (1 + Config| Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
                        prior(cauchy(0, 1), class = sd), #This is a regularizing prior, meaning we will allow the SD of the betas to vary across subjects
                        prior(cauchy(0, 1), class = sigma)), #overall variability that is left unexplained 
