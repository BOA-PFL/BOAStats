...
+--------------------+------------------------------------+
| Test Name          | AS_DaniellePractice_Agility_Jun24  |
+====================+====================================+
...
| **Test Type:**     | Practice                           |
+--------------------+------------------------------------+
| **Configurations** | Baseline: Lace Replacement         |
|                    |                                    |
|                    | Test configurations:               |
|                    |                                    |
|                    | -   PFS                            |
...
+==============+=====================+===============================+==============================+
| 2 Athletes   | Treadmill Running   | Pressure insoles              | Fit:                         |
|              |                     |                               |                              |
|              |                     |                               | -   Heel contact area        |
|              |                     |                               |                              |
...
presdat <- read.csv(file.choose())

#Load in Compiled Qualitative Sheet
#qualDat <- read_xlsx(file.choose())


baseline <- 'LR' # baseline configuration

otherConfigs <- c('PFS') # other configurations tested against base
...
```         
We have moderate confidence that PFS performed worse than LR ( 88 %)     Estimated difference: -1.2 to 22.3 % based on Milena's Compiled Data
We have moderate confidence that PFS performed worse than LR ( 86 %) 
     Estimated difference: -1.8 to 15.2 % based on Adam
```
...
```         
We have moderate confidence that PFS performed worse than LR ( 86 %)     Estimated difference: -12.4 to 1.6 % based on Milenas compiled agility data
We have moderate confidence that PFS performed worse than LR ( 87 %) 
     Estimated difference: -10.1 to 1.2 % based on Adam
```
...
```         
There were inconsistent differences between PFS and LR ( 42 %)       Estimated difference: -12.4 to 9.1 % based on Milena
We have moderate confidence that PFS performed worse than LR ( 84 %) 
 	 Estimated difference: -23.3 to 4.6 % based on Adam
```
...
```         
We have minimal confidence that PFS performed worse than LR ( 78 %)      Estimated difference: -3.1 to 10 % based on Milena
We have moderate confidence that PFS outperformed LR ( 85 %) 
 	 - Estimated difference: -25.7 to 3.8 % based on Adam
```
...
```         
We have minimal confidence that PFS performed worse than LR ( 79 %)      Estimated difference: -14.3 to 4.2 % based on Milena
We have moderate confidence that PFS outperformed LR ( 87 %) 
 	 - Estimated difference: -1.5 to 20.8 %
```
...
```         
We have minimal confidence that PFS performed worse than LR ( 71 %)      Estimated difference: -3.6 to 1.9 % based on Milena
We have minimal confidence that PFS performed worse than LR ( 75 %) 
 	 Estimated difference: -9 to 3.4 % based on adam 
```
...
ggplot(data = toedat, aes(x = peakPower)) + geom_histogram() + facet_wrap(~Subject) 

p<-withinSubPlot(toedat, colName = 'maxmaxToes', dir = 'lower')
p + ylab('Toe Pressure (KPA)')


toedatMod <- brm(data = toedat, # Bayes model
              family = gaussian,
              z_score ~ Config + (1 + Config| Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
...
extractVals(toedat, toedatMod, otherConfigs, baseline, 'maxmaxToes', 'lower') 
