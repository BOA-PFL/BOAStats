...
-   **The purpose of this test was to evaluate how a heel lock guide affects court performance in a single dial and dual dial configuration.**
...
H2: Improved fit will result in better speed, ie. [reduced contact time.]{.underline}

**Additional Benefits:**

H3: Reduced contact time will be achieved through increased propulsive force (i.e. in the direction of movement), and increased ankle force production in the direction of movement
...
ev_torque <- extractVals(torqueDat , torqueMod, otherConfigs, baseline, 'R_DialTorque1', 'higher')

es_torque <- EffectSizeLM(torqueDat, 'R_DialTorque1 ~ Config + (1 | Subject)')
...
ev_CT <- extractVals(cmjDatCT, cmjCTMod, otherConfigs, baseline,  'CT', dir = 'lower')

es_CT <- EffectSizeLM(cmjDatCT,'CT ~ Config + (Config|Subject)')

checkEffectMagnitude(ev_CT, es_CT, baseline)
```
...
ev_cmjPF <- extractVals(cmjDatPF, cmjPFMod, otherConfigs, baseline, 'peakGRF_Z', 'higher')

es_cmjPF <- EffectSizeLM(cmjDatPF, 'peakGRF_Z ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_cmjPF, es_cmjPF, baseline)
```
...
ev_cmjPP <- extractVals(cmjDatPP, cmjPPMod, otherConfigs, baseline, 'peakPFmom', 'higher')

es_cmjPP <- EffectSizeLM(cmjDatPP, 'peakPFmom ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_cmjPP, es_cmjPP, baseline)
```
...
ev_cmjPower <- extractVals(cmjDatPPow, cmjPowerMod, otherConfigs, baseline, 'peakPower', 'higher')

es_cmjPower <- EffectSizeLM(cmjDatPPow, 'peakPower ~ Config + Order + (1 + Config | Subject)')

checkEffectMagnitude(ev_cmjPower, es_cmjPower, baseline)
...
ev_skaterCT <- extractVals(skaterDatCT, skateCTMod, otherConfigs, baseline, 'CT', 'lower')

es_skaterCT <- EffectSizeLM(skaterDatCT, 'CT ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_skaterCT, es_skaterCT, baseline)
```
...
ev_skaterPF <- extractVals(skaterDatPF, skatePFMod, otherConfigs, baseline, 'peakGRF_X', 'higher')

es_skaterPF <- EffectSizeLM(skaterDatPF, 'peakGRF_X ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_skaterPF, es_skaterPF, baseline)
```
...
ev_skaterPow <- extractVals(skaterDatPow, skatePowMod, otherConfigs, baseline, 'peakPower', 'higher')

es_skaterPow <- EffectSizeLM(skaterDatPow, 'peakPower ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_skaterPow, es_skaterPow, baseline)
...
ev_kneeAbdROM <- extractVals(kneeAbdROM, kneeAbdROMMod, otherConfigs, baseline, 'kneeABDrom', 'lower')

es_kneeAbdROM <- EffectSizeLM(kneeAbdROM, 'kneeABDrom ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_kneeAbdROM, es_kneeAbdROM, baseline)
...
ev_ankEvVel <- extractVals(ankEvVel, ankEVvelMod, otherConfigs, baseline, 'peakEVvel', 'lower')

es_ankEvVel <- EffectSizeLM(ankEvVel, 'peakEVvel ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_ankEvVel, es_ankEvVel, baseline)
...
ev_COMp <- extractVals(COMp, COMwMod, otherConfigs, baseline, 'COMWork_pos', 'lower')

es_COMp <- EffectSizeLM(COMp, 'COMWork_pos ~ Config + Order + (1 + Config | Subject)')

checkEffectMagnitude(ev_COMp, es_COMp, baseline)
```
...
ev_COMn <- extractVals(COMn, NCOMwMod, otherConfigs, baseline, 'COMWork_neg', 'higher')

es_COMn <- EffectSizeLM(COMn, 'COMWork_neg ~ Config + Order + (1 + Config | Subject)')

checkEffectMagnitude(ev_COMn, es_COMn, baseline)
```
...
ev_HeelCon <- extractVals(HeelCondat, HeelMod, otherConfigs, baseline, 'heelAreaP', 'higher')

es_HeelCon <- EffectSizeLM(HeelCondat, 'heelAreaP ~ Config + ProxDial + (1 + Config | Subject)')

checkEffectMagnitude(ev_HeelCon, es_HeelCon, baseline)
```
...
ev_Toe <- extractVals(Toedat, ToeMod, otherConfigs, baseline, 'maxmaxToes', 'lower')

es_Toe <- EffectSizeLM(Toedat, 'maxmaxToes ~ Config + (1 + Config | Subject)')

checkEffectMagnitude(ev_Toe, es_Toe, baseline)
```
...
ev_pk <- extractVals(pkDat, pkMod, otherConfigs, baseline, 'maxDorsalPressure', 'lower')

es_pk <- EffectSizeLM(pkDat, 'maxDorsalPressure ~ Config + (1 | Subject)')

checkEffectMagnitude(ev_pk, es_pk, baseline)
```
...
ev_avg <- extractVals(avgDat, avgMod, otherConfigs, baseline, 'meanDorsalPressure', 'higher')

es_avg <- EffectSizeLM(avgDat, 'meanDorsalPressure ~ Config + (1 | Subject)')

checkEffectMagnitude(ev_avg, es_avg, baseline)
...
ev_ff <- extractVals(ffDat, ffDatMod, otherConfigs, baseline, 'ffDorsalPressure', 'higher')

es_ff <- EffectSizeLM(ffDat, 'ffDorsalPressure ~ Config + (1 | Subject)')

checkEffectMagnitude(ev_ff, es_ff, baseline)
...
ev_mf <- extractVals(mfDat, mfDatMod, otherConfigs, baseline, 'mfDorsalPressure', 'higher')

es_mf <- EffectSizeLM(mfDat, 'mfDorsalPressure ~ Config + (1 | Subject)')

checkEffectMagnitude(ev_mf, es_mf, baseline)
...
ev_ins <- extractVals(insDat, insDatMod , otherConfigs, baseline, 'instepDorsalPressure', 'higher')

es_ins <- EffectSizeLM(insDat, 'instepDorsalPressure ~ Config + (1 | Subject)')

checkEffectMagnitude(ev_ins, es_ins, baseline)
