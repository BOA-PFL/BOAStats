...
baselineDat <- subset(testDat, testDat$Shoe == 'Tri') # Change to baseline shoe name

newShoeDat <- subset(testDat, testDat$Shoe == 'Asym') # Change to shoe you want to compare to baseline
...
for (sub in unique(testDat$Subject)) {
   ...
   diff[r] <- mean(tempnewDat$z_score) - mean(tempbaseDat$z_score) # Change to variable of interest
...
ggscatter(corrDat, x = "Length (cm)", y = "diff", 
          ...
          xlab = 'Foot Length', ylab = 'Change in performance from Tri to Asym') 