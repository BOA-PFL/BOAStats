...
|                    |                                                 |
|                    | -   HL (High Upper, Low Midsole Structure)       |
|                    |                                                 |
...
![](images/PFL_ShoeStructure_Court_Group_1.jpg)
...
dat <- read.csv('Z:\\Testing Segments\\Material Testing\\StructureMaterialsA&S\\Overground\\CompiledAgilityDataTest.csv')

#Load in Compiled Qualitative Sheet
qualDat <- read_xlsx('C:\\Users\\adam.luftglass\\OneDrive - Boa Technology Inc\\General\\Testing Segments\\Material Testing\\StructureMaterialsA&S\\StructureMaterialsAS.xlsx')


# Load in the insole data
Pressdat <- read.csv('Z:\\Testing Segments\\Material Testing\\StructureMaterialsA&S\\XSensor\\Cropped\\0_CompiledResults_4.csv')

baseline <- 'HL' # baseline configuration
...
qualDat %>%
  group_by(Config)%>%
  summarize(
    R_Torque_Prox = mean(RDialTorque_1, na.rm = TRUE),
    #R_Torque_Dist = mean(R_DialTorque2, na.rm = TRUE),
    L_Torque_Prox = mean(LDialTorque_1, na.rm = TRUE)
   # L_Torque_Dist = mean(L_DialTorque2, na.rm = TRUE)
  )%>%
  gt()

ggplot(qualDat, aes(x=Config, y = LDialTorque_1, color = Config, group= Subject)) + 
  geom_point(size = 4)+ 
...
cmjDatPF<- subset(cmjDatPF, cmjDatPF$z_score < 2) #removing outliers  
cmjDatPF<- subset(cmjDatPF, cmjDatPF$z_score > -2)

ggplot(data = cmjDatPF, aes(x = peakGRF_Z, color = Config)) + geom_histogram() + facet_wrap(~Subject) 

p <- withinSubPlot(cmjDatPF, colName = 'peakGRF_Z', dir = 'higher')
...
skaterDatCT<- subset(skaterDatCT, skaterDatCT$z_score < 2) #removing outliers  
skaterDatCT<- subset(skaterDatCT, skaterDatCT$z_score > -2)
...
skaterDatPF<- subset(skaterDatPF, skaterDatPF$z_score < 2) #removing outliers  
skaterDatPF<- subset(skaterDatPF, skaterDatPF$z_score > -2)
...
extractVals(HeelCondat1, HeelCondat1Mod, otherConfigs, baseline, 'heelAreaP', 'higher')

p <- withinSubPlot(HeelCondat1, colName = 'heelAreaP', dir = 'higher')
p + ylab('Heel Contact (%)')


DorsalPress1 <- Pressdat %>% 
  group_by(Subject)  %>%
  mutate(z_score = scale(maxDorsalP)) %>% 
  group_by(Config)

DorsalPressDat1Mod <- brm(data = DorsalPress1, # Bayes model
              family = gaussian,
              z_score ~ Config + (1 + Config| Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
              prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                        prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
...
extractVals(DorsalPress1, DorsalPressDat1Mod, otherConfigs, baseline, 'maxDorsalP', 'lower')

p <- withinSubPlot(DorsalPress1, colName = 'maxDorsalP', dir = 'lower')
