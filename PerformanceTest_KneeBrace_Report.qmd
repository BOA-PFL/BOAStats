...
agilityDat <- read_csv('C:/Users/bethany.kilpatrick/Boa Technology Inc/PFL - General/Testing Segments/EndurancePerformance/EH_KneeBracePilotII_Aug23/Overground/CompiledAgilityData_3.csv')
...
withinSubQualPlot <- function(inputDF) {
  ...
  ggplot(data = whichConfig, mapping = aes(x = as.factor(Config), y = OverallConfidence, col = BestConfig, group = Subject)) + geom_point(size = 4) + 
    geom_line() + xlab('Configuration') + scale_color_manual(values=c("#000000", "#00966C", "#ECE81A","#DC582A","#CAF0E4")) + ylab('Rating') + theme(text = element_text(size = 36))
...
ggplot(braceDat, aes(x =Trial , y = RKneeBrace_VertPos,  color= Config, group = Config)) +
  geom_point(size=5)+
  # geom_line()+
...
braceDat %>%
  group_by(Config)%>%
  summarize(
    boaMean = mean(deltaFour, na.rm = TRUE),
    boaSD = sd(deltaFour, na.rm = TRUE)
  )

## Save as csv for BigData
write.csv(braceDat, "C:/Users/bethany.kilpatrick/Boa Technology Inc/PFL - General/Testing Segments/EndurancePerformance/EH_KneeBracePilotII_Aug23/Overground/Static/AllStaticBraceDat.csv", row.names=FALSE)
...
#Normalizing the brace displacement into z-scores 
...
jointDat %>%
  ...
  summarize(
    avgKneeFlexAn = mean(PkRkneeFlexAng),
    avgGRFZ = mean(peakGRF_Z),
    avgKneeAbdROM = mean(kneeABDrom),
    avgKneeADDmom = mean(PeakKneeAbMoment),
    avgKneeExtMom = mean(peakKneeEXTmom),
    avgFlexROM = mean(kneeFlexROM),
    avgAbdROM = mean(kneeABDrom)
  ) 

# Gut checking w/ lmer models 
fullMod <- lmer( peakKneeEXTmom ~ scalePos * Config  + (1|Subject) , data = jointDat) 
plot_model(fullMod)
plot_model(fullMod, type = 'int')
summary(fullMod)
...
runmod_cmj <- brm(data = jointDat, # Bayes model
              family = gaussian,
...
##### CMJ jump peak knee flexion angle
...
p <- withinSubPlot(cmjDat, colName = 'kneeFLEXrom', dir = 'higher')
...
extractVals(jointDat, 
runmod_cmj, otherConfigs, baseline, 'kneeFLEXrom', 'higher') 
...
jointDat<- subset(jointDat, jointDat$z_score < 2) #removing outliers
jointDat<- subset(jointDat, jointDat$z_score > -2)

ggplot(data = jointDat, aes(x = PeakKneeAbMoment, color = Config)) + geom_histogram() + facet_wrap(~Subject) 


p <- withinSubPlot(cmjDat, colName = 'PeakKneeAbMoment', dir = 'lower')
...
extractVals(jointDat, 
runmod_cmj, otherConfigs, baseline, 'PeakKneeAbMoment', 'lower') 
...
ggplot(data = jointDat, aes(x = kneeABDrom, color = Config)) + geom_histogram() + facet_wrap(~Subject) 
...
extractVals(jointDat, 
runmod_cmj, otherConfigs, baseline, 'kneeABDrom', 'lower')  
...
##### skater jump peak knee extension moment


p <- withinSubPlot(jointDat_Skate, colName = 'peakKneeEXTmom', dir = 'higher')
p + ylab('Peak Knee Extension Moment (Nm)')



jointDat_Skate <- jointDat_Skate %>% 
  #filter(impulse_Z < 1000) %>%
  group_by(Subject) %>%
  mutate(z_score = scale(peakKneeEXTmom)) %>% 
  group_by(Config)
...
runmod_skate <- brm(data = jointDat_Skate, # Bayes model
              family = gaussian,
              z_score ~ Trial + Config  + (Config|Subject), #fixed effect of configuration and time period with a different intercept and slope for each subject
               prior = c(prior(normal(0, 1), class = Intercept), #The intercept prior is set as a mean of 25 with an SD of 5 This may be interpreted as the average loading rate (but average is again modified by the subject-specific betas)
                      prior(normal(0, 1), class = b), #beta for the intercept for the change in loading rate for each configuration
...
extractVals(jointDat_Skate, 
runmod_cmj, otherConfigs, baseline, 'peakKneeEXTmom', 'higher') 



##### skater jump peak knee flexion angle
...
p <- withinSubPlot(jointDat_Skate, colName = 'kneeFLEXrom', dir = 'higher')
p + ylab('Knee Flexion ROM (deg)')
...
jointDat_Skate<- subset(jointDat_Skate, jointDat_Skate$z_score < 2) #removing outliers
jointDat_Skate<- subset(jointDat_Skate, jointDat_Skate$z_score > -2)

ggplot(data = jointDat_Skate, aes(x = PeakKneeAbMoment, color = Config)) + geom_histogram() + facet_wrap(~Subject) 


p <- withinSubPlot(jointDat_Skate, colName = 'PeakKneeAbMoment', dir = 'lower')
p + ylab('Peak Knee Abducion Moment (Nm)')
...
ggplot(data = jointDat_Skate, aes(x = kneeABDrom, color = Config)) + geom_histogram() + facet_wrap(~Subject) 


p <- withinSubPlot(jointDat_Skate, colName = 'kneeABDrom', dir = 'lower')
p + ylab('Knee Frontal Plane ROM (deg)')
