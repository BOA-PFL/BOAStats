...
fullMod <- lmer(EE_dist ~ AnkleNegWorkMean + AnklePosWorkMean + KneeNegWorkMean + KneePosWorkMean + HipNegWorkMean + HipPosWorkMean + 
              HipAbdROMMean + HipFlexROMMean + KneeAbdROMMean + KneeFlexROMMean + AnkleInvROMMean + AnkleFlexROMMean + 
              ankleMomXMean + ankleMomYMean + kneeMomXMean + kneeMomYMean + hipMomXMean + hipMomYMean + CTmean + pVGRFmean + pBFMean + pLFmean + pMFmean + Speed_minPerKm + (1|Subject), data = allDat)
...
fullModTrail <- lmer(EE_dist ~ HipAbdROMMean + HipFlexROMMean + KneeAbdROMMean + KneeFlexROMMean + AnkleInvROMMean + AnkleFlexROMMean + 
                  pVGRFmean + VALRmean + CTmean + (1|Subject), data = allDat)

summary(fullModTrail)

all_vifs <- car::vif(fullModTrail)
print(all_vifs)
...
while(any(all_vifs > 4)){
  ...
  signif_all <- signif_all[!(signif_all) %in% var_with_max_vif]
  myForm <- as.formula(paste("EE_dist ~",paste(signif_all, collapse=" + "), paste( "+ (1|Subject)"), sep = ""))
  fullModTrail <- lmer(myForm, data = allDat)
  all_vifs <- car::vif(fullModTrail)
...
redModTrail <- step(fullModTrail, reduce.fixed = TRUE, reduce.random = FALSE)
