...
ev_Speed <- extractVals(IMUdat, SpeedMod, otherConfigs, baseline, 'imuSpeed', 'higher')

es_Speed <- EffectSizeLM(IMUdat, 'imuSpeed ~ Config + Slope + Order + (Config|Subject)')
...
ev_Ever <- extractVals(Everdat, EverMod, otherConfigs, baseline, 'pIEgyro', 'lower')

es_Ever <- EffectSizeLM(Everdat, 'pIEgyro ~ Config + imuSpeed + Slope + Order + (1|Subject)')
...
es_Heel <- EffectSizeLM(HeelCondat, 'heelAreaP ~ Config + Slope + Order + (1|Subject)')
...
ev_SampEntX <- extractVals(xdat, xmod, otherConfigs, baseline, 'sampEntX_foot', 'higher')

es_SampEntX <- EffectSizeLM(xdat, 'sampEntX_foot ~ Config + Sesh + Speed + (1|Subject)')

checkEffectMagnitude(ev_SampEntX, es_SampEntX, baseline)
```
...
ev_SampEntY <- extractVals(ydat, ymod, otherConfigs, baseline, 'sampEntY_foot', 'higher')

es_SampEntY <- EffectSizeLM(ydat, 'sampEntY_foot ~ Config + Sesh + Speed + (1|Subject)')

checkEffectMagnitude(ev_SampEntY, es_SampEntY, baseline)
```
...
ev_SampEntZ <- extractVals(zdat, zmod, otherConfigs, baseline, 'sampEntZ_foot', 'higher')

es_SampEntZ <- EffectSizeLM(zdat, 'sampEntZ_foot ~ Config + Sesh + Speed + (1|Subject)')

checkEffectMagnitude(ev_SampEntZ, es_SampEntZ, baseline)
```
